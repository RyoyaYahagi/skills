...
9. owner 保存と Console 閲覧を分けて考える
- owner の共有データ本体は `private database` の custom zone に置かれる。owner 側の `shared database` が空でも直ちに異常ではない。
- `iCloud共有` 表示だけでサーバー保存成功と断定しない。保存直後に同じ zone をアプリ側で再読込し、`Family / Bag / Item` 件数をログへ出して確認する。
- `Fetch Changes` が 0 件でも、保存失敗とは限らない。change token や Console UI の制約で見えないことがある。

10. Console と `cktool query-records` は無条件 query を信用しない
- `Query Records` や `cktool query-records` は、フィルタなし query だと内部的に `recordName` を使って `Field 'recordName' is not marked queryable` で落ちることがある。
- `recordName` を追いかける前に、custom field の `id` や `name` を `QUERYABLE` にして、`name != ""` のような明示フィルタ付き query を試す。
- Console UI で見えなくても、`cktool` で queryable field を使った query が通るなら、保存自体は成功している可能性が高い。

11. orphan データと逆参照キャッシュを先に疑う
- ローカル件数が多いのに CloudKit 保存件数が少ない場合、まず `item.family == nil` / `bag.family == nil` の orphan 件数を確認する。
- 家族共有へ載せたい既存データが orphan なら、保存前に current family へ関連付けてから同期する。
- `family.resolvedItems` `family.resolvedBags` のような逆参照だけで保存対象を決めると、関係の更新直後に件数が欠けることがある。必要なら `modelContext` から `family.id` で直接 fetch して保存対象を決める。

12. 保存直後の再同期がローカルを削ることがある
- `syncFamilyStateFromCloudKit` が remote に存在しない local item を delete する実装なら、保存直後の再同期で local 件数が急減することがある。
- この症状が出たら、まず「本当に remote へ何件保存できているか」をログや `cktool` で確認し、その後に削除ロジックを疑う。
...
- Console では 0 件なのにアプリでは保存成功している:
`Fetch Changes` や `Query Records` UI を即座に信用せず、保存後の app-side 再読込件数と `cktool` の明示フィルタ query で実データを確認する。
