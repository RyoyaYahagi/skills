...
1. 作業開始時、および「次に何をするか」を判断する直前に、GitHubのOpen Issueを確認すること。`gh` が使える場合は `gh issue list --state open --limit 20` と着手候補 `gh issue view <番号>` を確認し、Issue内容に基づいて行動すること。
1. iOSアプリ実装（`*.xcodeproj`/`*.xcworkspace` が存在）では、ビルド成功後に `$appium-simulator-test` によるシミュレータE2Eスモークテストを必ず実行すること。コミット/報告はこの成功後に行うこと。
1. テスト実行で直せないバグ（失敗残存・タイムアウト）がある場合は、必ず GitHub Issue を登録すること。テスト中に追加すべき機能を発見した場合も、`enhancement` ラベルで Issue を登録すること。
