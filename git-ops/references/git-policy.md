...
- 物理的に作業を分離したい場合のみ `scripts/auto-worktree.sh [type] [description] [issue-number]` を実行する
- worktreeを使う場合は、既存worktreeを再利用し、必要時のみ新規作成する
...
1. **並行開発で作業ディレクトリ分離が必要な時**
   - 長時間並走するタスクを物理分離したい場合のみ作成する
2. **既存タスク再開時**
