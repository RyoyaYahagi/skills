...
Phase 4: 結果レポート
    ↓
Phase 5: GitHub Issue 自動作成 ※ 問題がある場合
```
...
- レビュー結果の出力を取得する
- 🔴 Critical が含まれる場合: **ユーザーに即報告する**（Phase 5 で Issue 化も行う）
...
- 🔴 Critical / 🟠 High が含まれる場合 → **Phase 5 で GitHub Issue 化**
...
# PR をレビューして Issue 化（Phase 1b + 5）
bash <skill-path>/scripts/run_codex_test.sh --pr 42

# PR レビュー + テスト（全フェーズ）
bash <skill-path>/scripts/run_codex_test.sh --pr 42 --base main

# GitHub Issue 作成を有効化（デフォルトは確認プロンプトあり）
bash <skill-path>/scripts/run_codex_test.sh --create-issues

# Medium 以上を全て Issue 化
